...
module.exports = function (public_key, private_key) {
	 ...
	 * @return String
	 */
	this.encode_object_base64 = function (object) {
		const data = new Buffer.from(JSON.stringify(object)).toString('base64')

		return data
	}

	/**
	 * Decode string from Base64 to UTF-8 string
	 *
	 * @param String $string
	 *
	 * @return String
	 */
	this.decode_base64 = function (string) {
	...
	this.api = function (path, params, callback, callbackerr) {
		...
		const data = this.encode_object_base64(params)
	...
	this.cnb_form = function (params) {
		let language = params.language ? params.language : 'ru'
